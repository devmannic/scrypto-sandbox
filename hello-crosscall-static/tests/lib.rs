...
fn test_hello() {
    ...
    let component = receipt1.component(0).unwrap(); // only one component is instantiated in this -static version
    ...
    // can't go direct to component 2 because it doesn't exist (and also visibility is pub(crate)
    let is_none_component = receipt1.component(1).is_none();
    assert!(is_none_component);
}
